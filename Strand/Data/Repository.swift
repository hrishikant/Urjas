...
// WHOOP wins, Ūrjas-computed fills the days it doesn't cover, and Apple Health only fills declared-
...
final class Repository: ObservableObject {
    ...
    /// "WHOOP import / Ūrjas computed / Apple Health" captions instead of a silent merged row.
    ...
    /// for a Ūrjas Charge/Rest on an import-only day, exactly like a live day (#823). These carry no raw HR
    ...
    enum TimelineMetric: String, CaseIterable, Identifiable, Sendable {
        ...
        var title: String {
            ...
            // NOT a stage Ūrjas trusts as truth — the pill names it "Band Sleep State" so it can't be
    ...
    /// explicit per `sourceCandidates`: imported WHOOP > Ūrjas-computed > declared-compatible Apple Health.
    ...
    static func sourceCandidates(forKey key: String, preferredSource: String,
        ...
        if preferredSource == whoopSource || preferredSource == actualWhoopSource {
            ...
            // `imported WHOOP > Ūrjas-computed` order. The computed sibling used to sit ahead of the
    ...
    /// Whether the Ūrjas-computed strap source may fill an Apple-preferred metric. Only the two daily
