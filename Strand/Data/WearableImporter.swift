...
/// any sleep score) are stored under REFERENCE metric keys only — never as Ūrjas's Charge/Effort/Rest.
/// Ūrjas recomputes its own scores downstream from the imported raw RHR / HRV / sleep, exactly as it
...
enum WearableImporter {
    ...
    static func importExport(url: URL, into store: WhoopStore,
        ...
        // Day rollups → DailyMetric. HRV/recovery/respiration that the export lacks stay nil; Ūrjas
        ...
        // browseable but never mistaken for a Ūrjas score.
        ...
        for d in result.days {
            ...
            // Reference-only: the brand's own scores, never a Ūrjas Charge/Effort/Rest input.
