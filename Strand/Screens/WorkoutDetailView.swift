...
//
// A READ-ONLY drill-down for one tapped session, built ONLY from the locked Ūrjas component system
            ...
            NoopCard(tint: StrandPalette.effortColor) {
                HStack(alignment: .center, spacing: 18) {
                    ZStack {
                        ...
                        VStack(spacing: 0) {
                            // The session's Effort contribution ticks up to its value — the Ūrjas signature.
...
// tiles locally and the route itself is on-device — Ūrjas never sends the route anywhere.
