...
struct TodayView: View {
    ...
    // TODAY's in-progress Effort (Ūrjas 0–100 axis), recomputed over the day's HR (local-midnight→now)
    ...
    /// the strap's real `deviceId`. ANY Ūrjas-computed strap sibling (a "-noop"-suffixed id, not just the
    /// active strap's) reads "On-device" — matching by suffix so a computed row from a non-active strap
    /// can't fall through to `FusionSource.noopComputed`'s raw "Ūrjas" displayName; the imported strap source
    ...
    // MARK: Apple Watch provenance (M1): "the watch is the sensor, Ūrjas is the brain"
    ...
    /// Today hero wording names the provider that supplied the score inputs, not where Ūrjas ran the math.
    ...
    /// baseline (mirrors the recovery seed gate), the insight then falls back to the state word.
    ///
    /// STOPGAP (#696): Ūrjas mixes HRV measurement methods on the shared `avgHrv` field,     /// strap/WHOOP-CSV HRV is RMSSD (~20-100 ms) while Apple-Health-imported HRV is SDNN
    ...
    /// The effective Effort strain (Ūrjas 0–100 axis) the gauge shows. For TODAY this prefers the live
    ...
    private func effortStrain(_ d: DailyMetric?) -> Double? {
        #if DEBUG
        // DEBUG promo harness: pin Effort (Ūrjas 0–100 axis) to the active frame's value. This single
    ...
    /// `strain` is on Ūrjas's 0–100 Effort axis; `UnitFormatter.effortValue` converts it to the
    ...
    /// The Effort gauge's scale maximum, 100 on Ūrjas's native axis, 21 on the WHOOP axis. Drives
    ...
    /// "Charge" marker (Ūrjas's name for recovery) at wake time (sleep end), else at the window start.
    ...
    private var sourcesSummaryRow: some View {
        ...
        .accessibilityHint("Show what Ūrjas is synced from")
    }
    ...
    /// strap doesn't transmit steps, so Ūrjas estimates them from motion calibrated to the phone's count;
    ...
    private func loadDayScoped() async {
        ...
        // badge names the sensor/import provider rather than the device that ran Ūrjas's math.
