...
/// Intelligence — Ūrjas's own recovery/strain/sleep scores, computed on-device from raw strap data
...
struct IntelligenceView: View {
    ...
    var body: some View {
        ...
        ScreenScaffold(title: "Intelligence",
                       subtitle: "Ūrjas scores your recovery, strain and sleep itself: on-device, no cloud.",
                       lazy: true,
                ...
                VStack(spacing: 14) {
                    ...
                    VStack(alignment: .leading, spacing: 10) {
                        Text("You'll likely wake around \(Int(f.charge.rounded())) ± \(Int(f.band.rounded())) Recovery if you sleep about \(sleepHoursLabel(f.plannedSleepHours)) tonight.")
                            ...
                            .fixedSize(horizontal: false, vertical: true)
                            .frame(maxWidth: .infinity, alignment: .leading)
                        Text("Estimate from today's strain, your typical sleep and your \(f.nights)-night recovery baseline, not a measurement. Your real Recovery is scored from tomorrow's HRV when you wake.")
                            .font(StrandFont.footnote).foregroundStyle(StrandPalette.textTertiary)
                            .fixedSize(horizontal: false, vertical: true)
                            .frame(maxWidth: .infinity, alignment: .leading)
                    }
        ...
        NoopCard(padding: 18, tint: StrandPalette.chargeColor) {
            VStack(alignment: .leading, spacing: NoopMetrics.cardInnerSpacing) {
                HStack {
                    ...
                    // The REAL source of the day's dashboard headline, not a hard-coded "Ūrjas-computed".
                    // The By-Day numbers are always Ūrjas's on-device scores, but when an import covers the
