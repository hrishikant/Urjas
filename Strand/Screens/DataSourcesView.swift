...
struct DataSourcesView: View {
    ...
    // own source, refresh. The brand's own scores are stored as reference only, never Ūrjas scores.
    ...
    // "Broadcast heart rate" (opt-in, OFF by default): make Ūrjas a standard BLE Heart Rate peripheral
    // (0x180D / 0x2A37) so a gym treadmill / Zwift / Peloton can read the live strap HR Ūrjas receives.
    ...
    /// score is NEVER mapped to a Ūrjas Charge/Effort/Rest — Ūrjas recomputes its own from the raw inputs.
    ...
    private var broadcastHrCard: some View {
        ...
        return card(title: String(localized: "Broadcast HR from this phone"), icon: "dot.radiowaves.up.forward",
            ...
            Text("Acts as a standard Bluetooth heart-rate strap. Pair Ūrjas from your treadmill, bike or app to see your strap's heart rate there.")
                .font(StrandFont.footnote)
                .foregroundStyle(StrandPalette.textTertiary)
                .fixedSize(horizontal: false, vertical: true)

            // FI-2 (#490) — the 4.0-vs-5.0 explainer. Broadcast works for BOTH strap generations because it
            // re-shares whatever LIVE heart rate Ūrjas already has off the strap; it doesn't depend on the
    ...
    private var generationExplainer: some View {
        VStack(alignment: .leading, spacing: 6) {
            ...
            generationRow(title: "WHOOP 5.0 & MG",
                          detail: String(localized: "Broadcasts the live heart rate Ūrjas derives from the strap once connected."))
        }
        .padding(.top, 2)
