...
//   1 — Derived on-device from raw by Ūrjas (computed recovery/strain/sleep from strap streams).
...
public enum MetricArbitrationPolicy {
    ...
    public static func tier(metric: MetricKind, source: FusionSource) -> Int {
        ...
        case .steps:
            ...
            case .noopComputed:  return 3   // Ūrjas step estimate from motion
            case .nutritionCsv:  return 3
            case .localCache:    return 3
            }

        case .sleep:
            // The best STAGER wins: imported WHOOP stages > Ūrjas-computed stages > phone sleep buckets.
            switch source {
            case .whoopImport:   return 0
            case .noopComputed:  return 1
    ...
    /// `sourceCandidates`: imported WHOOP first, then Ūrjas-computed, then phone (Apple before Health
