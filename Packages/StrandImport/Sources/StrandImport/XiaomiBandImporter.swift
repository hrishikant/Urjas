...
/// the daily-rollup tables and the per-session `sleep` table into Ūrjas's shapes.
///
/// This is **parse-only**: it never touches Ūrjas's own store, only the foreign file
...
public struct XiaomiBandImporter {
    ...
    // Day-rollup tables, keyed by Ūrjas day. (`goal_day` is intentionally skipped —
