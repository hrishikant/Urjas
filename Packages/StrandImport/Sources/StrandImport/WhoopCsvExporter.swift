...
/// Serializes Ūrjas's own cached rows back into WHOOP's 4-CSV export shape so Ūrjas's OWN importer
...
/// literal "UTC+00:00" timezone column: Ūrjas stores epoch seconds and tz-less day strings, so UTC
...
public enum WhoopCsvExporter {
    ...
    /// Stage minutes recovered from any persisted stagesJSON shape Ūrjas has ever written:
    ...
    public static func cyclesCSV(days: [DailyMetric],
        ...
        for d in days.sorted(by: { $0.day < $1.day }) {
            ...
            let cols: [String] = [
                ...
                // WHOOP-format and a Ūrjas→Ūrjas round-trip is lossless (importer scales it back up).
    ...
    public static func sleepsCSV(_ sessions: [CachedSleepSession],
        ...
        for s in sessions.sorted(by: { $0.startTs < $1.startTs }) {
            ...
            let cols: [String] = [
                ...
                // Ūrjas never stores a nap flag — everything exports as a main sleep so the importer
