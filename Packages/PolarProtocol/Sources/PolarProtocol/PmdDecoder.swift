...
//
// Polar's PMD service is proprietary but PUBLICLY DOCUMENTED (Polar's official BLE SDK). This is Ūrjas's
...
//
// This package decodes the **PPI** stream (measurement 0x03), which is the one Ūrjas actually needs: it
// yields heart rate AND the peak-to-peak (inter-beat) interval per beat — the same signal WHOOP R-R gives
// — so Ūrjas can compute HRV from a Polar H10 / OH1 / Verity Sense without running peak detection on raw
...
/// A PMD measurement type (the subset Ūrjas recognises). An unrecognised type decodes to `nil` rather than
...
public struct PolarPpiSample: Equatable, Sendable {
    ...
    /// The peak-to-peak / inter-beat interval in milliseconds — Ūrjas's HRV input (≈ an R-R interval).
    ...
    /// Raw flags bit1. Named per the Polar SDK's `skinContactStatus` (set = contact). NOTE: Ūrjas's
...
public enum PolarPmdDecoder {
    ...
    /// carries a measurement type Ūrjas doesn't recognise (no wrong guess).
